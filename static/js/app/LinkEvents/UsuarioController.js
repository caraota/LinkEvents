...
LinkEventsModule.controller('VListarUsuariosController', 
      ...
      $scope.ConfirmarAsist = function(usuario) {
         
          LinkEventsService.AConfirmarAsist({"eventId" : $routeParams['eventoid']}, {"user" : usuario}).then(function (object) {
            var msg = object.data["msg"];
            if (msg) flash(msg);
            var label = object.data["label"];
            if (label == '/ListarUsuarios/'+$routeParams['eventoid']) {
                $route.reload();
            } else {
                $location.path(label);
                $route.reload();
